...
## Interview Prep override

Use `sh <skill-dir>/scripts/excalidraw-v2.sh <command>` for every CLI command.
It pins the local v2.0.0 build to `http://127.0.0.1:3032` and disables implicit
auto-start. Ignore the upstream `npx`, global-install, and port `3000` examples
below. Prefer `excalidraw_live` MCP tools when loaded, and keep one canvas tab.
