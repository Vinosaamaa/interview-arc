...
description: Excalidraw canvas toolkit for creating, editing, and refining diagrams on the Interview Prep live canvas. Use for architecture or system-design drawing, iterative describe/screenshot refinement, Excalidraw and image export, snapshots, Mermaid conversion, or element-level canvas operations. Prefer the configured excalidraw_live MCP; otherwise use the bundled pinned-v2 wrapper. Never download or launch another Excalidraw package.
---
...
The local support runtime pins `mcp-excalidraw-server` v2.0.0 and its canvas to
`http://127.0.0.1:3032`. The wrapper resolves it from
`$INTERVIEW_PREP_SUPPORT_ROOT` or
`$HOME/Projects/interview-prep-support`.

- Prefer the configured `excalidraw_live` MCP tools when loaded.
- Otherwise resolve `scripts/excalidraw-v2.sh` relative to this `SKILL.md` and
  run it with `sh` in place of every `npx -y mcp-excalidraw-server` example
  below.
- Never use `npx`, install a global package, connect to port `3000`, start a
  second server, or use the removed remote Excalidraw MCP.
- Keep exactly one canvas tab connected; multiple tabs can duplicate bound
  label elements.
- Live WebSocket state is not durable. Export or checkpoint before treating a
  diagram as saved.
...
1. **MCP tools** — if `excalidraw_live/*` tools (e.g. `batch_create_elements`) are in your tool list, prefer them: results land directly in your context, and screenshots come back as images without touching disk.
2. **Pinned CLI wrapper** (default when no MCP tools are present):
   ```bash
   sh <skill-dir>/scripts/excalidraw-v2.sh <command>
   ```
   It runs only the project-pinned build and targets the existing loopback
   canvas. `start` explicitly starts that one server; other commands never
   auto-start another copy.
3. **REST API** (last resort, e.g. from application code): HTTP endpoints on `http://127.0.0.1:3032` — see `references/cheatsheet.md` for payloads. The server must already be running.

Open `http://127.0.0.1:3032` in exactly one browser tab. Screenshots, image
export, Mermaid conversion, and viewport control need that tab (CLI exit code
4 means it is missing).
...
- **Exit code 4 (browser required)?** Open `http://127.0.0.1:3032` in the one Playwright Chromium tab, then retry — screenshots, image export, viewport, and Mermaid conversion render in the frontend.
