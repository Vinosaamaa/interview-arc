...
export async function validateMcpToolAllowlists({
  ...
  const sharedSources = await Promise.all(["editorial-tools.ts", "leetcode-tools.ts", "coding-tools.ts", "drawing-tools.ts", "coaching-tools.ts", "solution-publication-tools.ts", "study-resource-tools.ts", "lecture-player-tools.ts", "study-resource-uploader.ts"].map(name => readFile(new URL(`mcp-worker/${name}`, repositoryRoot), "utf8")));
