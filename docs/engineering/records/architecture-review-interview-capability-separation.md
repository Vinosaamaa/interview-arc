...
status: proposed
title: Separate interview evidence ingest, Loop operations, and site-wide AI assistance
repository: interview-arc
capabilityIds: ["arc-interview-loops"]
createdAt: 2026-08-20
reconstructed: false
confidence: verified
unknowns: ["Interview-evidence retention, package limits, initial format allowlists, scanning policy, and transcript-body storage threshold are not yet selected.","The assistant provider, retention and region policy, owner budgets, conversation lifecycle, and initial domain adapters are not yet selected."]
modules: ["loop-interview-evidence-ingest","loop-command-module","sitewide-ai-assistant"]
interfaces: ["interview-evidence-package","loop-authoring-command","assistant-context-adapter","assistant-action-proposal"]
seams: ["browser-upload-to-private-evidence-package","website-adapter-to-loop-command","assistant-shell-to-domain-read-adapter","approved-proposal-to-domain-command"]
adapters: ["private-r2-asset-adapter","manual-loop-web","loop-recorder","assistant-provider-gateway","assistant-domain-adapter"]
...
run: null
---
# Separate interview evidence ingest, Loop operations, and site-wide AI assistance

Three user needs had been incorrectly modeled as one Loop-centered AI program: uploading recordings and transcripts from real interviews, creating and managing Loops in the website, and using AI throughout Interview Arc. That coupling was false. The architecture now treats them as independently owned product capabilities with separate GitHub issues, delivery paths, privacy decisions, and release criteria.

## Context

The website reads published Loop content and owner-private state but does not currently let the owner upload real interview evidence or create a Loop. Those are two ordinary product gaps. A third gap is that AI assistance is available through external coding or CLI workflows rather than through the authenticated website. Solving that third gap inside the Loop feature would make the assistant artificially Loop-specific and risk making deterministic operations depend on a model provider.
...
Interview evidence upload, Loop operations, and site-wide AI assistance are three separate capabilities:

1. Issue #415 owns explicit browser selection, resumable validation, storage, assignment, reading, revision, export, reconciliation, and governed deletion of private interview recordings and supplied transcripts. It does not invoke an AI or speech provider.
2. Issue #417 owns an ordinary deterministic Add Loop workflow and a shared `create_loop` domain command used by the website and existing Loop Recorder. It works with AI disabled.
...
An interview-evidence package groups assets from one interview event. The owner explicitly selects files in the browser and may assign the package to an existing Loop/Round or leave it in an unassigned inbox. D1 owns package, asset, supplied-transcript revision, assignment, upload-session, and command-receipt state. Private R2 owns quarantined and verified source bytes under opaque locators.

Finalization reconciles signature, MIME, size, checksum, D1 metadata, R2 object state, and authorized readback. Partial packages remain visible and recoverable. Supplied transcripts preserve original bytes and provenance and are parsed deterministically. This capability does not scan local folders or submit content to a model provider.

## Loop command boundary

The website Add Loop form and the existing Loop Recorder are separate authorization adapters over one versioned command. `create_loop` owns schema and domain validation, atomic creation of the Loop and Role Brief revision 1, initial stages and provenance, optimistic concurrency, idempotency, and stable receipts. Unknown values remain explicit and are not inferred.
...
The first release grounds published content, then proves a second distinct workspace. Command proposals are added only around existing deterministic commands. Loops join after #417. Private interview evidence joins only after #415 and a separate provider-privacy decision.
...
Three issue-owned implementations create more explicit interfaces and release records, but each vertical slice is independently useful, testable, recoverable, and removable. Manual functionality remains available during provider outage or policy disablement. Evidence privacy decisions remain isolated from assistant-provider decisions.
...
Issue #417 may ship whenever the deterministic Loop command and website adapter are complete. Issue #415 may ship against existing Loops plus an unassigned inbox. Issue #418 may begin with published-content grounding without waiting for either. Later adapter connections are separate reviewed changes.

This architecture review is the public-safe record of the separation decision. Each implementation receives its own worktree, branch, pull request, verification, numbered Engineering receipt, production release evidence, and post-merge cleanup. No private audio, transcript, prompt, response, owner identifier, or local filesystem path belongs in Git, GitHub, public logs, screenshots, or Engineering narrative.

## Design artifacts

The consolidated review is [`interview-capabilities-plan.html`](../../design/interview-capability-separation/interview-capabilities-plan.html). The detailed capability contracts are in the neighboring Markdown files. Both architecture views have editable draw.io sources, clean PNG previews, and round-trip editable PNGs.
