# Separate private interview evidence, Loop operations, and site-wide AI assistance

Status: proposed architecture spanning three independent issues:

- [#415 — Upload and attach private interview recordings and transcripts to Loops](https://github.com/Vinosaamaa/interview-arc/issues/415)
...
This design corrects an earlier false coupling. Uploading interview evidence is not an AI feature. Creating a Loop is not an AI feature. The AI assistant is a separate website-wide capability that may later consume both through reviewed adapters.

## The three questions

### 1. How do I upload an interview recording and transcript?

Interview Arc needs a private website uploader owned by issue #415. The owner selects files in the browser, reviews a package manifest, uploads/resumes them, confirms an exact Loop/Round or leaves them unassigned, and reads or deletes the evidence through owner-scoped D1/R2 state.

No AI is required. A supplied transcript is primary evidence. Optional future transcription or analysis is a separate derived operation and separate authorization.

Detailed plan: [`interview-evidence-upload.md`](interview-evidence-upload.md).

### 2. How do I add a Loop?

Today, use the long-lived `Interview Arc — Loop Recorder` task and provide explicit company, role, job-description/source, location, opened date, stages, dates, statuses, and unknowns. The website currently reads Loops but does not create them.
...
Issue #418 adds **Ask Interview Arc** to the shared website shell. It is not owned by Loops. It can operate across Today, Banks, practice, Learn, Review, Loops, and owner-selected private evidence as each domain exposes safe read or command interfaces.
...
```text
┌──────────────────────────────┐
│  #415 Private evidence       │  Upload/read/attach/delete files
│  Works without AI            │
└──────────────┬───────────────┘
...
| Foundation | #415 Evidence | #417 Loops | #418 Assistant |
| --- | --- | --- | --- |
| Cloudflare Access | Derives owner scope | Derives owner scope | Derives owner scope/context |
| D1 | Package metadata, transcript revisions, links | Loop, Role Brief, stages, receipts | Conversations, selected context, proposals, approvals |
| R2 | Private source bytes | No source-media ownership | No direct storage authority; selected private content only through adapters |
| Domain commands | Assign/delete evidence | Create/revise Loop | Calls allowlisted commands only after approval |
...
2. #415 can ship with existing Loops and an unassigned inbox; it does not wait for #417.
...
- Owner-private recordings, transcripts, prompts, responses, and IDs never enter Git, public logs, issues, fixtures, screenshots, or Engineering narrative.
