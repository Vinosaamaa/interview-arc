...
test("the repository MCP allowlist matches the Worker registration catalog in order", async () => {
  const [workerSource, editorialSource, leetcodeSource, codingSource, drawingSource, coachingSource, publicationSource, resourceSource, lectureSource, uploaderSource, repositoryConfig] = await Promise.all([
    ...
    readFile(new URL("mcp-worker/study-resource-uploader.ts", repositoryRoot), "utf8"),
  ...
  assert.deepEqual(parseEnabledTools(repositoryConfig), [
    ...
    ...parseRegisteredTools(uploaderSource),
  ]);
});
