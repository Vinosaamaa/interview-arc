...
test("Past loads and renders Final tailored answer separately after Conversation", async () => {
  ...
  const conversation = source.indexOf('id="case-transcript"');
  const finalAnswer = source.indexOf('id="case-final-answer"');
  const codeAttempts = source.indexOf('id="case-code-attempts"');
  assert.ok(conversation >= 0 && finalAnswer > conversation && codeAttempts > finalAnswer);
});

test("Problem Bank Past-attempt navigation opens the exact activity snapshot", async () => {
  const source = await readFile(new URL("app/home-client.tsx", root), "utf8");
  assert.match(source, /selectedProblemAttempts\.map\(\(entry\) => <button key=\{entry\.id\} onClick=\{\(\) => openAttemptFromSolution\(entry\)\}/);
  assert.match(source, /const selectedEntryActivityId = .*artifact\?\.activityId/);
});
