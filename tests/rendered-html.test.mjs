...
test("the Cloudflare build contains the Interview Arc dashboard", async () => {
  ...
  assert.match(bundle, /finished yesterday/);
...
test("the refined analytics and composer layouts keep their intended grouping", async () => {
  ...
  assert.match(client, /aria-label=\{`Atmosphere: \$\{atmosphereMode\}\. Switch atmosphere`\}/);
  assert.match(client, /title=\{`Atmosphere: \$\{atmosphereMode\}\. Switch atmosphere`\}/);
  assert.doesNotMatch(client, /atmosphere-toggle-label/);
  assert.doesNotMatch(client, /"Atmosphere off"/);
