...
function InterviewMotif() {
  return (
    <svg className="workspace-identity-motif" viewBox="0 0 24 24" aria-hidden="true">
...
export function WorkspaceIdentityBadge({
  ...
  selected?: boolean;
  unavailable?: boolean;
  onSelect?: (workspace: WorkspaceIdentityId) => void;
}) {
  const { label, title } = WORKSPACE_IDENTITY[workspace];
  const ariaLabel = unavailable ? `${label}, later` : label;
  return (
    <button
      type="button"
      className={`workspace-identity-badge workspace-identity-${workspace}${selected ? " active" : ""}${unavailable ? " later" : ""}`}
      aria-label={ariaLabel}
      aria-current={selected ? "page" : undefined}
      aria-disabled={unavailable ? "true" : undefined}
      disabled={unavailable}
      title={unavailable ? `${title} unavailable` : title}
      onClick={() => onSelect?.(workspace)}
    >
      <span className="workspace-identity-mark" aria-hidden="true">{MOTIFS[workspace]}</span>
      <strong className="workspace-identity-name">{label}</strong>
      {unavailable ? <small className="workspace-identity-later">Later</small> : null}
    </button>
  );
}
