...
async function read<T>(url: string, signal?: AbortSignal): Promise<T> { const r = await fetch(url, { signal }); const value = await r.json(); if (!r.ok)
...
function MaterialReader({ material: m }: {
  ...
  <details className="material-original" open={expanded} onToggle={e => { const open = e.currentTarget.open; setExpanded(open); if (open && !source && !busyRef.current)
   ...
   {expanded && <div className="material-original-body"><p>The original is preserved separately from these notes.</p><a href={`/api/study-resources?resourceId=${m.resourceId}&original=1`} download>Download unchanged original</a>
    ...
    {source && source.resource.chunkCount > 0 && <><label htmlFor="transcript-filter">Find in loaded source</label><input id="transcript-filter" value={filter} onChange={e => { setFilter(e.target.value); setVisibleParts(10); }} placeholder="Word or phrase"/><p className="material-muted">{fragments.length} of {source.resource.chunkCount} source parts loaded{source.nextChunk === null ? " · Complete reading copy" : ""}</p>
    <div className="material-transcript">{matching.slice(0, visibleParts).map(f => <section key={f.ordinal}><h4>{f.location} · part {f.ordinal + 1}</h4><pre>{f.text}</pre></section>)}</div>
