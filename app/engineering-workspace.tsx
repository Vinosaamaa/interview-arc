...
export function EngineeringIcon({ name }: { name: EngineeringIconName }) {
  const paths = (() => {
    ...
    if (name === "index") return <><rect x="4.5" y="5" width="6.5" height="14" rx="1.2" /><path d="M14 8.5h5.5M14 12h5.5M14 15.5h3.5" /></>;
    if (name === "evidence") return <><rect x="13" y="5" width="6.5" height="14" rx="1.2" /><path d="M4.5 8.5h6M4.5 12h6M4.5 15.5h4" /></>;
...
function RecordReader({
  ...
  return <article ref={readerRef} className="engineering-reader engineering-record-panel" aria-labelledby="engineering-record-title">
    <nav className="engineering-contents-nav" aria-label="Record contents">
      ...
      <div className="engineering-reader-panel-actions" role="group" aria-label="Workbench panels">
        <button type="button" className="engineering-panel-toggle" onClick={onToggleIndex} aria-expanded={indexOpen} aria-controls="engineering-index" aria-label={indexOpen ? "Hide index" : "Open index"}>
          <EngineeringIcon name="index" />
          <span>Index</span>
        </button>
        <button type="button" className="engineering-panel-toggle" onClick={onToggleEvidence} aria-pressed={evidenceOpen} aria-label={evidenceOpen ? "Hide evidence" : "Open evidence"}>
          <EngineeringIcon name="evidence" />
          <span>Evidence</span>
        </button>
      </div>
    </nav>
...
function EngineeringPanelClose({ onClose }: { onClose: () => void }) {
  return <button type="button" className="engineering-panel-close" onClick={onClose} aria-label="Close evidence">Close</button>;
}

function EngineeringEvidencePanel({ record, index, onSelect, onClose, hidden }: { record: EngineeringJournalRecord; index: EngineeringJournalIndex; onSelect: (ref: string) => void; onClose: () => void; hidden: boolean }) {
  ...
  return <aside className="engineering-evidence-panel" aria-label="Evidence and lineage" inert={hidden} aria-hidden={hidden}>
    <header><div><span>Evidence desk</span><h2>Exact evidence</h2></div><EngineeringPanelClose onClose={onClose} /></header>
...
function EngineeringEmptyEvidencePanel({ view, onClose, hidden }: { view: EngineeringView; onClose: () => void; hidden: boolean }) {
  return <aside className="engineering-evidence-panel engineering-evidence-empty" aria-label="Exact evidence" inert={hidden} aria-hidden={hidden}>
    <header><div><span>Evidence desk</span><h2>Exact evidence</h2></div><EngineeringPanelClose onClose={onClose} /></header>
...
export default function EngineeringWorkspace({ index, view, onNavigateView }: { index: EngineeringJournalIndex; view: EngineeringView; onNavigateView: (view: EngineeringView) => void }) {
  ...
  const select = (ref: string) => { setSelectedRef(ref); setContentsSection("overview"); closeIndex(); };
  ...
  return <div className={`engineering-destination engineering-destination-${view}`}><EngineeringDestinationHero index={index} view={view} /><section ref={workbenchRef} className={`engineering-workspace ${mobileReaderOpen ? "index-closed" : "index-open"} ${displayEvidence ? "evidence-open" : "evidence-closed"}`} onKeyDown={(event) => {
    if (event.key !== "Escape" || !narrowWorkbench) return;
