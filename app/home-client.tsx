    ...
    <main className={`app-shell active-view-${activeWorkspace === "engineering" ? "engineering" : view} active-workspace-${activeWorkspace} active-destination-${activeWorkspace === "engineering" ? engineeringView : activeWorkspace === "learn" ? learnDestination : view}`} aria-hidden={arrivalState !== "entered"}>
      ...
      <aside className="sidebar">
        ...
        {activeWorkspace === "learn"
          ? <nav className="primary-nav learn-local-nav" aria-label="Learn navigation">{LEARN_NAV_ITEMS.map(([id, label], index) => <button key={id} className={learnDestination === id ? "active" : ""} aria-current={learnDestination === id ? "page" : undefined} onClick={() => navigateToLearn(id)}><span>{String(index + 1).padStart(2, "0")}</span><span className="nav-copy">{label}</span></button>)}</nav>
          : activeWorkspace === "interview"
            ? <nav className="primary-nav" aria-label="Interview navigation">{INTERVIEW_NAV_ITEMS.map(([id, label], index) => <button key={id} className={view === id ? "active" : ""} aria-current={view === id ? "page" : undefined} onClick={() => navigateToPrimaryView(id)}><span>{String(index + 1).padStart(2, "0")}</span><span className="nav-copy">{label}</span></button>)}</nav>
