...
export default function HomeClient({ content, today }: { content: ContentIndex; today: string }) {
  ...
  const composerFinishedCount = composerQuestionEntries.filter((entry) => Boolean(entry.latestAttempt)).length;
  const composerFinishedPercent = composerQuestionEntries.length
    ? Math.round((composerFinishedCount / composerQuestionEntries.length) * 100)
    : 0;
      ...
      {composer.open && <div className="modal-backdrop" role="presentation" onMouseDown={() => setComposer(EMPTY_COMPOSER)}>
        <section className="composer" role="dialog" aria-modal="true" aria-labelledby="composer-title" onMouseDown={(event) => event.stopPropagation()}>
          ...
          {composer.mode === "session" && !composer.editingId ? <div className="session-composer">
            ...
            <div className="activity-picker-heading"><span>{composer.type === "leetcode" ? "Search the bank or paste a LeetCode URL" : `Search the ${typeLabel(composer.type).toLowerCase()} bank, paste a URL, or type a new title`}</span>{hasComposerFilters && <button type="button" className="filter-clear" onClick={clearComposerFilters}>Clear</button>}<div className="bank-progress-meter activity-picker-progress" style={{ background: `conic-gradient(var(--signal-dark) ${composerFinishedPercent}%, #e4e9e1 ${composerFinishedPercent}% 100%)` }} aria-label={`${composerFinishedCount} of ${composerQuestionEntries.length} ${typeLabel(composer.type).toLowerCase()} questions finished`}><span><strong>{composerFinishedCount}</strong><small>of {composerQuestionEntries.length}</small></span></div></div>
