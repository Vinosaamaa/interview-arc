...
export async function openPdfPages(file: Blob) {
    ...
    return { total: pdf.numPages, close: () => pdf.loadingTask.destroy(), async render(pageNumber: number) {
            ...
            const scale = Math.min(2, Math.sqrt(4000000 / (base.width * base.height))), viewport = page.getViewport({ scale }), canvas = document.createElement("canvas");
            canvas.width = Math.ceil(viewport.width);
            canvas.height = Math.ceil(viewport.height);
            try {
                await page.render({ canvas, viewport }).promise;
                return await new Promise<Blob>((resolve, reject) => canvas.toBlob(v => v ? resolve(v) : reject(Error("Page image could not be rendered.")), "image/png"));
            }
            finally {
                canvas.width = 0;
                canvas.height = 0;
...
export async function* pdfPageImages(file: Blob) {
    ...
    try {
        if (pdf.total > 100)
            throw Error("Original saved. Page-image preparation supports up to 100 pages per PDF; split a reading copy for longer documents.");
