...
export type LearningMaterial = ReturnType<typeof item> & {
...
function sourceUrl(value: string | undefined, kind: string) {
    ...
    if (u.protocol !== "https:" || u.username || u.password)
        throw new ResourceError("Use a public HTTPS source URL without credentials.");
    if (kind === "youtube" && !/^(www\.|m\.)?youtube\.com$|^youtu\.be$/.test(u.hostname))
        throw new ResourceError("Use the original YouTube video URL.");
    return u.href;
}
export async function publishLearningMaterial(db: ResourceDatabase, bucket: ResourceBucket, owner: string, input: unknown) {
    ...
    if (JSON.stringify(v.summary).length > 180000)
        throw new ResourceError("Summary exceeds 180,000 characters; split the material into focused publications.", 413);
    const { object, resource } = await readStudyOriginal(db, bucket, owner, v.resourceId);
    if (resource.sourceSha256 !== v.sourceSha256)
        throw new ResourceError("The original identity does not match. Read the resource again.", 409);
    if (await resourceHash(new Uint8Array(await object.arrayBuffer())) !== v.sourceSha256)
        throw new ResourceError("Original verification failed. Restore the original before publishing.", 503);
    if (v.kind === "youtube" && resource.chunkCount === 0)
        throw new ResourceError("Save an actual readable transcript before publishing this video.");
    ...
    if (prior) {
        if (prior.fingerprint !== fingerprint)
            throw new ResourceError("This publish identity already contains different material. Use a new operation for a revised publication; the original is preserved.", 409);
        return { ...item(prior), duplicate: true };
    }
    ...
    if (!saved || saved.fingerprint !== fingerprint)
        throw new ResourceError("Publication conflicted. Read the existing material before retrying.", 409);
...
export async function getLearningMaterial(db: ResourceDatabase, owner: string, id: string): Promise<LearningMaterial> {
    materialIdSchema.parse(id);
    const row = await db.prepare("SELECT * FROM learning_materials WHERE owner_id=? AND material_id=?").bind(owner, id).first<Row>();
    if (!row)
        throw new ResourceError("Learning material not found.", 404);
    return { ...item(row), summary: JSON.parse(row.summary_json) };
}
export async function listLearningMaterials(db: ResourceDatabase, owner: string, query = "", offset = 0) {
    if (query.length > 300 || !Number.isSafeInteger(offset) || offset < 0 || offset > 100000)
        throw new ResourceError("Invalid material search.");
    const rows = await db.prepare("SELECT material_id,title,kind,source_url,resource_id,source_sha256,coverage,limitations,created_at FROM learning_materials WHERE owner_id=? AND (?='' OR instr(lower(title),lower(?))>0 OR instr(lower(summary_json),lower(?))>0) ORDER BY created_at DESC,material_id LIMIT 21 OFFSET ?").bind(owner, query, query, query, offset).all<Row>();
    return { materials: rows.results.slice(0, 20).map(item), nextOffset: rows.results.length > 20 ? offset + 20 : null };
}
export async function getLearningMaterialSource(db: ResourceDatabase, owner: string, id: string) {
    const m = await getLearningMaterial(db, owner, id), source = await getStudyResource(db, owner, m.resourceId, 0, m.sourceSha256);
    if (source.resource.sourceSha256 !== m.sourceSha256)
        throw new ResourceError("Source identity changed. Reload before reading.", 409);
    const rows = await db.prepare("SELECT ordinal,location,text_offset AS offset,body AS text FROM study_resource_chunks WHERE owner_id=? AND resource_id=? AND extraction_sha256=? ORDER BY ordinal").bind(owner, m.resourceId, source.resource.extractionSha256).all<{
        ordinal: number;
        location: string;
        offset: number;
        text: string;
    }>();
    if (rows.results.length !== source.resource.chunkCount)
        throw new ResourceError("The reading copy is incomplete; the original is still available.", 503);
    return { resource: source.resource, fragments: rows.results, nextChunk: null };
}
