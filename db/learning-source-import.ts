...
export async function importLearningSource(db:ResourceDatabase,bucket:ResourceBucket,owner:string,input:{url:string;title:string;operationId:string},fetcher:typeof fetch=fetch){
 ...
 if(/(^|\.)youtube\.com$|^youtu\.be$/.test(url.hostname))return importYouTube(db,bucket,owner,input,url,fetcher);
...
export function captionTracks(html:string):{baseUrl:string;languageCode?:string;kind?:string}[]{
 const match=/"captionTracks"\s*:\s*\[/.exec(html);if(!match)return [];
 const start=match.index+match[0].length-1;let depth=0,quoted=false,escaped=false;
 for(let i=start;i<html.length;i++){const c=html[i];if(quoted){if(escaped)escaped=false;else if(c==="\\")escaped=true;else if(c==='"')quoted=false;continue;}if(c==='"'){quoted=true;continue;}if(c==='[')depth++;if(c===']'&&--depth===0){try{const value=JSON.parse(html.slice(start,i+1));return Array.isArray(value)?value.filter(v=>v&&typeof v.baseUrl==='string'):[];}catch{return [];}}}
 return [];
}
async function importYouTube(db:ResourceDatabase,bucket:ResourceBucket,owner:string,input:{title:string;operationId:string},url:URL,fetcher:typeof fetch){
 const id=url.hostname==="youtu.be"?url.pathname.slice(1):url.pathname==="/watch"?url.searchParams.get("v"):null;
 if(!id||!/^[-_a-zA-Z0-9]{11}$/.test(id))throw new ResourceError("Supply a YouTube watch URL for one video.");
 ...
 if(!track)throw unavailable();
 const captions=new URL(track.baseUrl);
 if(captions.protocol!=="https:"||captions.username||captions.password||captions.port||!["www.youtube.com","youtube.com"].includes(captions.hostname)||captions.pathname!=="/api/timedtext")throw unavailable();
 captions.searchParams.set("fmt","vtt");
 const r=await fetcher(captions,{redirect:"error",signal:AbortSignal.timeout(30000)});if(!r.ok||!r.body)throw unavailable();
 const bytes=new Uint8Array(await new Response(boundedResourceStream(r.body,MAX_RESOURCE_BYTES).stream).arrayBuffer()),text=new TextDecoder().decode(bytes);
 if(!text.startsWith("WEBVTT")||!text.includes(" --> "))throw unavailable();
