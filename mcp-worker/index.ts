...
async function requestVoiceDeliveryRetry(ownerId: string, activityId: string, env: Env) {
  const before = await readVoiceDeliveryBlockers(ownerId, activityId);
  ...
  const signalPublished = await publishOwnerLiveUpdate(env.LIVE_UPDATES, ownerId, "voice_capture");
  return {
    activityId,
...
function createServer(ownerId: string, env: Env, ctx: ExecutionContext) {
  ...
  server.registerTool(
    ...
    async ({ activityId }) => {
      const result = await requestVoiceDeliveryRetry(ownerId, activityId, env);
      return {
