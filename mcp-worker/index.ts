...
function createServer(ownerId: string, env: Env, ctx: ExecutionContext, chatgpt = false) {
  ...
  server.registerTool(
    ...
    async ({ activityId }) => {
      const result = await requestVoiceDeliveryRetry(
        ...
        () => publishOwnerLiveUpdate(
          env.LIVE_UPDATES,
          ownerId,
          `voice_delivery_retry:${activityId}`,
          { awaitDelivery: true },
        ),
      );
      return {
  ...
  server.registerTool(
    "revise_loop",
    {
      description: "Append one owner-authorized revision for Loop process metadata, arbitrary ordered stages or groups, explicit dates/statuses/outcomes, owner-provided format/interviewers, and concise per-question owner approach, review, or rating. Legacy round-level assessment/feedback and exact/reconstructed memory remain parseable but are not solicited or inferred. The company-and-role identity remains stable; dates never infer completion or outcome. Only the Loop Recorder may mutate this administrative record.",
  ...
  server.registerTool(
    "query_loops",
    {
      description: "Read bounded owner-private Loops, display-safe Role Brief revisions, and factual Journey aggregates. Current and exact historical revisions preserve owner-authored stage context, per-question approach/review, legacy round notes, and legacy exact/reconstructed memory while excluding raw job descriptions and private Role Brief notes; aggregates count only explicit Loop, stage, date, and outcome records.",
  ...
  server.registerTool(
    ...
    async (input) => {
      try {
        ...
        await publishOwnerLiveUpdate(env.LIVE_UPDATES, ownerId, "learning", { executionContext: ctx });
        return {
