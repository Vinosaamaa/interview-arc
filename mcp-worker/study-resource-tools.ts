...
import { z } from "zod";
import { registerStudyResourceUploader } from "./study-resource-uploader.ts";
...
export function registerStudyResourceTools(server:McpServer,db:ResourceDatabase,bucket:ResourceBucket,owner:string){
  registerStudyResourceUploader(server);
